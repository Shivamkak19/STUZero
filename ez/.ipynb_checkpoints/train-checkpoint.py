...
def start_ddp_trainer(rank, config):
    assert rank >= 0
    print(f'start {rank} train worker...', flush=True)
    agent = agents.names[config.agent_name](config)         # update config
    manager = None
    num_gpus = torch.cuda.device_count()
    num_cpus = multiprocessing.cpu_count()
    print(f'[DEBUG] Calling ray.init with {num_gpus} GPUs, {num_cpus} CPUs...', flush=True)
    ray.init(num_gpus=num_gpus, num_cpus=40, object_store_memory=40 * 1024 * 1024 * 1024 if config.env.image_based else 20 * 1024 * 1024 * 1024)
    print('[DEBUG] ray.init() completed', flush=True)
    set_seed(config.env.base_seed + rank >= 0)              # set seed
    print('[DEBUG] set_seed completed', flush=True)
    # set log
    ...
    if rank == 0:
        ...
        if config.ddp.training_size == 1:
            ...
            print(f'[DEBUG] about to call wandb.init with name={wandb_name}', flush=True)
            logger = wandb.init(
                name=wandb_name,
