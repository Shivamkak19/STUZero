...
class OSFPredictor(nn.Module):
    def __init__(self, T, h, m, D_in, D_out):
        '''
            :param T: int, input sequence length
            :param h: int, number of spectral filters
            :param m: int, number of AR terms
            :param D_in: int, input (action) dimension
            :param D_out: int, output (state) dimension
        '''
        super().__init__()
        self.T = T
        self.h = h
    ...
    def forward(self, u_seq, y_seq):
        '''
            Input u_seq: (B, T, D_in)
            Input y_seq: (B, T, D_out)
